...
if __name__ == '__main__':
    ...
    df_on = st.sidebar.toggle("Determining Factor")
    if df_on:
        determining_factor = st.sidebar.number_input("Determining Factor", value=5.6)

    flow_on = st.sidebar.toggle("Flow")
    if flow_on:
        flow = st.sidebar.number_input("Flow", value=22.5)

    rp_on = st.sidebar.toggle("Required Pressure")
    if rp_on:
        required_pressure = st.sidebar.number_input("Required Pressure", value=16.14)
        square_root_rp = math.sqrt(required_pressure)

    col1.subheader("DF", divider = True)
    col1.caption("The equivalent K at a node.")
    col2.subheader("Flow (gpm)", divider = True)
    col2.caption("This is the calculated flow.")
    col3.subheader("RP (psi)", divider = True)
    ...
    if df_on & rp_on:
        ...
        flow_display = flow_calculation_box.metric(label="Calculated", value=flow_calc_formatted)
    if df_on & flow_on & rp_on:
    ...
    if df_on & flow_on:
        ...
        rp_display = rp_calculation_box.metric(label="Calculated", value=rp_calc_formatted)
    if df_on & flow_on & rp_on:
